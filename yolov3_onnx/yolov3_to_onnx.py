...
#
#关于onnx创建模型和加载权重等流程和相关函数参考https://www.cnblogs.com/vh-pg/p/11736174.html
...
class DarkNetParser(object):
    ...
    def __init__(self, supported_layers):
        ...
        # parameters:
        #OrderedDict是一个有序的字典，会对其中字典对象进行排序，按照输入的顺序进行排序
    ...
    def _next_layer(self, remainder):
        ...
        #确定当前层的layer_name,zfill在指定的字符串开头添加0，直到达到指定的长度
        ...
        if layer_type in self.supported_layers:
            ...
            for param_line in layer_param_lines:
                ...
                if param_line[0] == '#':
                    continue
                #提取对应的参数类型和具体的值，_parse_params参考本文件下的实现
    ...
    def _parse_params(self, param_line):
        ...
        if param_type == 'layers':
            ...
            #isinstance来判断一个对象是否是一个已知的类型
            #isalpha判断字符串是否只有字母构成
        elif isinstance(param_value_raw, str) and not param_value_raw.isalpha():
            #isdigit方法检测字符串是否只由数字组成。
...
class GraphBuilderONNX(object):
    ...
    def __init__(self, output_tensors):
        ...
        self.batch_size = 1
    #创建一个包含权重的onnx图，遍历所有的层配置，创建一个onnx图，并用权重填充
    ...
    def _make_onnx_node(self, layer_name, layer_dict):
        ...
        #如果当前层次没有input_tensor
        if self.input_tensor is None:
            #是"net"层的话。。
            if layer_type == 'net':
                #make_input_tensor参考本文件下的实现
                #从"net"层来创建一个输入张量
                major_node_output_name, major_node_output_channels = self._make_input_tensor(
                    layer_name, layer_dict)
                #MajorNodeSpecs参考本文件下的实现
                #存储相应的output_name和对应的输出通道数，比呢且标记相应层次结点是否被创建
    ...
    def _make_input_tensor(self, layer_name, layer_dict):
        ...
        #创建一个onnx输入张量
        #helper.make_tensor_value_info通常用于创建一个tensor的信息，onnx中的函数
        input_tensor = helper.make_tensor_value_info(
            str(layer_name), TensorProto.FLOAT, [
                batch_size, channels, height, width])
        self.input_tensor = input_tensor
        return layer_name, channels
        #获取之前创建的onnx结点，跳过没有生成的结点
...
def main():
    ...
    # DarkNetParser将会从这些层次里面提取参数。。但是不会处理"yolo"层，因为"yolo"层会被放在后处理模块进行处理
