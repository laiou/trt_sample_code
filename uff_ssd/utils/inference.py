...
#inference_utils.TRTInference(args.trt_engine_path, ssd_model_uff_path,trt_engine_datatype=args.trt_engine_datatype,batch_size=args.max_batch_size)
#用于管理tensorrt推理过程中的相关数据结构
class TRTInference(object):
    ...
    def __init__(self, trt_engine_path, uff_model_path, trt_engine_datatype=trt.DataType.FLOAT, batch_size=1):
        ...
        #初始化并注册所有现有的TensorRT插件到IPluginRegistry
        ...
        if not os.path.exists(trt_engine_path):
            ...
            #这里的build_engine参考engine.py下的实现
            #用于创建一个引擎
